...
const reducer = (state = initialState.customDataDownload, action) => {
  switch (action.type) {
    ...
    case types.REMOVE_APPLIED_FILTER:
      const filter =
        action.removal.opHours && !action.removal.removeAll
          ? (el) => el.values[0] !== "Operating Hours Only"
          : (el) => el.key !== action.removal.removedFilter;

      return {
        ...state,
        appliedFilters: action.removal.removeAll
          ? []
          : state.appliedFilters.filter(filter),
      };
    case types.LOAD_HOURLY_EMISSIONS_SUCCESS:
      return Object.assign(
        {},
        state,
        { dataPreview: action.hourlyEmissions.data },
        { totalCount: action.hourlyEmissions.totalCount }
      );
    case types.LOAD_OZONE_EMISSIONS_SUCCESS:
      return Object.assign(
        {},
        state,
        { dataPreview: action.ozoneEmissions.data },
        { totalCount: action.ozoneEmissions.totalCount }
      );
    case types.LOAD_ANNUAL_EMISSIONS_SUCCESS:
      return Object.assign(
        {},
        state,
        { dataPreview: action.annualEmissions.data },
        { totalCount: action.annualEmissions.totalCount }
      );
    default:
      return state;
  }
