...
import * as actions from "../actions/bulkDataFilesActions";
...
describe("bulk data files Reducer", () => {
  it("should update state when load buld data files actions are dispatched", () => {
    let state, action;
    action = actions.loadBulkDataFilesSuccess(dataTableRecords);
    state = bulkDataFilesReducer(initialState.bulkDataFiles, action);
    expect(state.dataTable.length).toEqual(dataTableRecords.length);
  });
});
