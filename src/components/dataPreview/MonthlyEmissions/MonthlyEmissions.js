...
export const MonthlyEmissions = ({
  ...
  useEffect(() =>{
    if(dataPreview === null){
      loadMonthlyEmissionsDispacher(filterCriteria);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  },[]);
  ...
  const data = useMemo(() => {
    if (loading === 0 && dataPreview !== null) {
      return getMonthlyEmissionsTableRecords(dataPreview);
    } else {
      return [{ col3: "Loading monthly emissions data preview..." }];
    }
  }, [loading, dataPreview]);
...
const mapDispatchToProps = (dispatch) => {
  return {
    loadMonthlyEmissionsDispacher: (filters) => dispatch(loadMonthlyEmissions(filters))
  };
};
