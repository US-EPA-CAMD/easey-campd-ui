...
export const HourlyEmissions = ({
  ...
  useEffect(() =>{
    if(dataPreview === null){
      loadHourlyEmissionsDispacher(filterCriteria);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  },[]);
  ...
  const data = useMemo(() => {
    if (loading === 0 && dataPreview !== null) {
      return getHourlyEmissionsTableRecords(dataPreview);
    } else {
...
const mapDispatchToProps = (dispatch) => {
  return {
    loadHourlyEmissionsDispacher: (filters) => dispatch(loadHourlyEmissions(filters))
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(HourlyEmissions);
